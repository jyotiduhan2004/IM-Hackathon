"""Landing-page generators — home, changes, domain hubs.
...
the public names at the bottom of that module so existing callers keep working.
...
def _generate_changes(wiki_dir: Path, db_conn: Any | None = None) -> Path:
    ...
    New behavior: group the 60 most-recently-updated topic/system/policy
    pages by calendar day (UTC), newest first, with a wikilink per page.
    ...
    if not recent:
        ...
        for md_file, recency in recent:
            day = datetime.fromtimestamp(recency, tz=UTC).strftime("%Y-%m-%d")
            by_day.setdefault(day, []).append((md_file, recency))

        for day in sorted(by_day, reverse=True):
            lines.append(f"## {day}")
            lines.append("")
            for md_file, _recency in by_day[day]:
                category = md_file.parent.name
                lines.append(f"- [[{category}/{md_file.stem}]]")
            lines.append("")
