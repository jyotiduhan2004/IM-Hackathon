"""entity_write_autoheal middleware — nudge raw entity writes toward create_entities.

When the agent uses `write_file` or `edit_file` on `/wiki/entities/<slug>.md`
(or `/wiki/people/<slug>.md` once Tier P lands), the write succeeds but we
append a hint to the returned `ToolMessage` content so the next turn sees
a reminder to prefer `create_entities(email=..., display_name=...)` — which
generates a deterministic email-canonical slug and initialises the stub.

Rationale: for three concrete production failures (see
docs/BACKLOG.md → "coordinators verify, llms propose"), letting the LLM
invent entity slugs produced:
...
# Paths we consider entity-ish — anchored at virtual roots. Leading-slash
# form is what the sandbox presents post-path_autoheal.
...
def _is_entity_write(tool_name: str, args: dict[str, Any]) -> tuple[bool, str]:
    """Return (is_entity_write, target_path)."""
...
class EntityWriteAutohealMiddleware(AgentMiddleware):
    """Append a create_entities nudge to raw writes on entity/people pages."""
    ...
    def wrap_tool_call(
        ...
        is_entity, target = _is_entity_write(tool_name, request.tool_call.get("args") or {})
