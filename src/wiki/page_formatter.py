...
Provides `format_page` (and supporting helpers) so the post-batch
coordinator can normalize a page in-process without shelling out to the
CLI. The CLI wrapper in ``scripts/format_wiki.py`` imports from here.

Responsibilities:
- Strip H2 sections whose title matches an "agent-written nav" pattern
  (Related / People / Team / Related Topics / Related Systems / Related
  Entities / Related People / Key Contributors / Contributors).
- Preserve `## Related Work` — that's content, not nav.
- Regenerate ONE `## Related` section at the end from the union of:
    * frontmatter `related:` list
    * inline `[[slug]]` links in the body that resolve to real wiki pages
- Idempotent: running twice produces byte-identical output on a clean page.

Direction: `src/coordinator/post_batch.py` imports `format_page` from
here. Scripts may import too — there is no reverse dependency.
"""
...
def format_page(path: Path, wiki_root: Path, confirm: bool = False) -> bool:
    ...
    this one file". The post-batch coordinator hook uses this so it can run
